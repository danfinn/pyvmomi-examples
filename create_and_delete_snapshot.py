...
inputs = {'vcenter_ip': '10.110.3.12',
          'vcenter_password': 'your_password_here',
          'vcenter_user': 'dfinn',
          'snapshot_name' : 'pyvmomi_snapshot',   
          'ignore_ssl': True
          }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Create, delete or list snapshots in VSphere.')

    parser.add_argument('-n', '--name',
                        required=True,
                        action='store',
                        help='VM name, as displayed in vshpere.')

    parser.add_argument('-a', '--action',
                        required=True,
                        action='store',
                        help='Action to take.  Options are: "create, remove or list".')
    args = parser.parse_args()

    if not args.action:
       print "No action was given, this is required"
       raise SystemExit
    ...
    try:
        ...
        if operation == 'create':
            ...
            print "Creating snapshot on %s" % vm_name
            ...
            for snapshot in snapshots:
                if snapshot_name == snapshot.name:
                    ...
                    print ("Removing snapshot %s on %s"  % (snap_obj, vm_name))
